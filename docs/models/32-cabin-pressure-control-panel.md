...
Only the signal of the gauge servo goes through a screw terminal. Its **+5 V** and **GND** sit on the pin headers of this board. The servo plug has to be rewired before it will work - see [Outflow Valve Gauge](31-outflow-valve-gauge.md#wiring).

The five wires of the FLT ALT display module are soldered straight into its input pads and reach this board; the output of that module feeds the LAND ALT module through a Dupont cable. Which pad is which is on the [Cabin Pressure Display](../pcb/cabin-pressure-display.md) page.
